...
public class OrderDao implements IOrderDao {
	...
	public OrderListDTO getOrderById(int id) {
		dlsContext.select().from(OrderList.ORDER_LIST.join(OrderItem.ORDER_ITEM).onKey().join(Product.PRODUCT).onKey())
		.where(OrderList.ORDER_LIST.ID.eq(id)).fetch().map(DTOMappers.prodMapper);
		
		OrderListRecord fetchOne = dlsContext.fetchOne(OrderList.ORDER_LIST, OrderList.ORDER_LIST.ID.eq(id));
		if (fetchOne == null){
			return null;
		}
		return fetchOne.map(DTOMappers.orderMapper);
	}
